...
unplayed = unplayed[unplayed.Date <= datetime(2012,06,05)]
    ...
    for match in data.index:
        ...
        try:
            if data.get_value(match,'HomeTeam') == 0:
                ...
                resultcsv.append(pdf_home[binn][1:])
        except IndexError, e:
            print binn, rankdiff
            raise e
...
for piece in range(int(segments)-1):
    ...
    # pdf = np.append(
            # np.append(
                # np.append(
                    # np.transpose([foo[0][1][:-1]]),
                    # np.transpose([(map(float, foo[3][0] + foo[0][0]) /
                                   # (foo[0][0]+foo[1][0]+foo[2][0]+foo[3][0]+foo[4][0]+foo[5][0])
                                  # )])
                    # ,1),
                # np.transpose([(map(float,foo[4][0] + foo[1][0]) /
                               # (foo[0][0]+foo[1][0]+foo[2][0]+foo[3][0]+foo[4][0]+foo[5][0])
                              # )])
                # ,1),
            # np.transpose([np.append(map(float,foo[2][0])/(foo[0][0]+foo[1][0]+foo[2][0]),0)]),1)
