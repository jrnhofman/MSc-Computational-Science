...
# 0.4772 with probabilities (bin-width = 0.1, iter = 150, weight = 200)

# 0.5688 by taking into account higher ranked players winning/losing AGAINST a hometeam
# factor = 20
# iter = 300
# bin_width = 0.1
# bins = 65

# 0.5683, 0.5705, 0.5624, 0.5654, 0.5667
# iter = 600, 150, 50, 100, 200

# iter = 150: 0.5701, 0.5729, 0.5694, 0.5705
# weight = 200, 1, 0.1, 20

# iter = 150, weight = 1: 0.5690, 0.5701, 0.5570, 0.5402, 0.5714, 0.5714
# bin_width = 0.1, 1, 10, 0.01, 0.10, 0.25
...
def spread(rank):
    ...
    for team1,team2 in zip(test_teams, test_teams[1:]):
        ret.append(red_outcome(team1, team2, rank))
    return ret
...
def prediction(data,rank,pdf_nohome,pdf_home,pdf_ahome):
    ...
    for match in data.index:
        ...
        if signn == 1:
            ...
            predicted_outcome = 1 - (1 - argmax(pdf[binn][1:]) / 2.0)
        ...
        # if signn == 1:
        #     pred_value += pdf[binn][1 + int((1-data.get_value(match,'Outcome'))*2)]
        # else:
        #     pred_value += pdf[binn][1 + int(data.get_value(match,'Outcome')*2)]
...
for piece in range(int(segments)-1):
    ...
    #Pdf without distinction between hometeams or not (#NOT CORRECT!)
