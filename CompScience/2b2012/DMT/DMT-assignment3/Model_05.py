...
def prediction(data,rank,pdf_nohome,pdf_home,pdf_ahome):
    ...
    for match in data.index:
        datee = data.get_value(match,'Date')
        t1 = data.get_value(match,'Team1')
        t2 = data.get_value(match,'Team2')
        rank1 = rank.get_value(index[t1],'Rank')
        rank2 = rank.get_value(index[t2],'Rank')
        ...
        if signn == 1:
            ...
            resultcsv.append([datee,t1,t2,a,b,c])
        else:
            predicted_outcome = 1 - (1 - argmax([a,b,c]) / 2.0)
            resultcsv.append([datee,t1,t2,c,b,a])
    ...
    with open('resultthijs.csv', 'w') as f:
        for d,t1,t2,w,t,l in resultcsv:
            f.write("%s \t %s \t \t %s \t \t %s \t %s \t %s\n" % (d,t1,t2,w,t,l))
...
if TRAIN_ON_EVERYTHING:
    ...
    unplayed = unplayed[unplayed.Date <= datetime.datetime(2012,06,30)]
