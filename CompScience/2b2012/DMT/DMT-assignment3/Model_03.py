...
# 0.4789 by not taking into account higher ranked players winning/losing AGAINST a hometeam
...
def histograms(data, rank):
    ...
    #Criteria for not being a hometeam, only counts highest ranked players as hometeam!
    wins = data[((data.Sign == 1) & (data.Outcome == 1.0) & (data.HomeTeam == 0)) | ((data.Sign == -1) & (data.Outcome == 0.0) & (data.HomeTeam == 0)) | ((data.Sign == -1) & (data.Outcome == 0.0) & (data.HomeTeam == 1))]
    ties = data[(data.Outcome == 0.5) & (data.HomeTeam == 0)]
    losses = data[((data.Sign == 1) & (data.Outcome == 0.0) & (data.HomeTeam == 0)) | ((data.Sign == -1) & (data.Outcome == 1.0) & (data.HomeTeam == 0)) | ((data.Sign == -1) & (data.Outcome == 1.0) & (data.HomeTeam == 1))]
    ...
    #Criteria for being a hometeam WHEN BEING HIGHEST RANKED
...
def prediction(data,rank,pdf_nohome,pdf_home):
    ...
    for match in data.index:
        ...
        if signn == 1 and data.get_value(match,'Outcome') == 1 and data.get_value(match,'HomeTeam') == 1:
            pdf = pdf_home
            count += 1
        elif data.get_value(match,'Outcome') == 0.5 and data.get_value(match,'HomeTeam') == 1:
            pdf = pdf_home
            count += 1
        elif signn == 1 and data.get_value(match,'Outcome') == 0 and data.get_value(match,'HomeTeam') == 1:
            pdf = pdf_home
            count += 1
            #if ((data.get_value(match,'HomeTeam') == 1) and not ((signn == -1) | (data.get_value(match,'Outcome') != 0.5))):
            #pdf = pdf_home
            #count += 1
        else:
            pdf = pdf_nohome
...
for piece in range(int(segments)-1):
    ...
    #Predict matches
    prediction(data_testing,rank,pdf_nohome,pdf_home)
