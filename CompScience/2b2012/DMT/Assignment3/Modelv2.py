...
def histograms(data, rank):
    ...
    binlist = [(-5.2 + 0.2*i) for i in range(54)]
...
def prediction(data,rank,pdf,pdf_home):
	...
	for match in data.index:
		...
		binn = int(floor((5.2 + floor(rankdiff/0.2)*0.2)/0.2))
		if data.get_value(match,'HomeTeam') == 0:
			...
			if data.get_value(match,'Outcome') == 1:
				pred_value += pdf_home[binn][1]
			elif data.get_value(match,'Outcome') == 0.5:
				pred_value += pdf_home[binn][2]
			else:
				pred_value += pdf_home[binn][3]
...
for piece in range(int(segments)-1):
    ...
    pdf = np.append(np.append(np.append(np.transpose([foo[0][1]]),np.transpose([np.append(map(float,foo[0][0])/(foo[0][0]+foo[1][0]+foo[2][0]),0)]),1),np.transpose([np.append(map(float,foo[1][0])/(foo[0][0]+foo[1][0]+foo[2][0]),0)]),1),np.transpose([np.append(map(float,foo[2][0])/(foo[0][0]+foo[1][0]+foo[2][0]),0)]),1)
	...
	print prediction(data_testing,rank,pdf,pdf_home)
...
#                 continue
#         successful_tests += 1
#         prediction = pred_outcome(t1, t2, rank)
