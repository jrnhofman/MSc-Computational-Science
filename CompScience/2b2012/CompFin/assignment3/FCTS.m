function price = FCTS(S0,K,r,sigma,deltat,deltax)
    ...
    b1 = (r-0.5*sigma*sigma)*deltat/(2*deltax) + 0.5*sigma*sigma*deltat/(deltax*deltax);
    b0 = 1 - sigma*sigma*deltat/(deltax*deltax) - r*deltat;
    bm1 = -(r-0.5*sigma*sigma)*deltat/(2*deltax) + 0.5*sigma*sigma*deltat/(deltax*deltax);
    ...
    for i=1:n+1
        ...
        %V(i,N+2) = exp(M2)-K;
        %V(i,N+2) = (exp(M2)-K)*exp(-r*((i-1)*deltat));
