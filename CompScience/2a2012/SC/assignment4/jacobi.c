...
void compute(void)
{
    ...
    for(j=1; j<N-1; j++)
    {
	(*newval)[1][j] = 0.25*((*curval)[2][j] + (*curval)[0][j] + (*curval)[1][j-1] + (*curval)[1][j+1]);
	(*newval)[npoints][j] = 0.25*((*curval)[npoints+1][j] + (*curval)[npoints-1][j] + (*curval)[npoints][j-1] + (*curval)[npoints][j+1]);
    }
	...
	for(j=1; j<N-1; j++)
	{
	    (*newval)[i][j] = 0.25*((*curval)[i+1][j] + (*curval)[i-1][j] + (*curval)[i][j-1] + (*curval)[i][j+1]);
	}
    }
...
void visualize(int iter)
{
    ...
    FILE *fp;
    char file[256];
    snprintf(file, 256, "data.%djac%.0e",parms.ntotal,parms.threshold);    
    fp = fopen(file, "w");
