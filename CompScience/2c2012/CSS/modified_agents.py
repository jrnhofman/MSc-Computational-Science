...
###################################################
# Modified: Active agents move to the quadrant    #
# with the most other active agents, so that they #
# are better protected and have less chance of    #
# getting arrested. If the agent is not active,   #
# it moves randomly, the same holds if there are  #
# no active agents in site.                       #
###################################################
    ...
    def act(self,vision,quad):
        ...
        if self.maxJail == -1:
            ...
            if self.active == 2:
                ...
                q, maxx = 0,0
                arr=roll(roll(grid,shift=-self.position[0]+agentVision,axis=0),shift=-self.position[1]+agentVision,axis=1)[:visionLength,:visionLength] #Cut out vision of agent

                #Select the quadrant with maximum number of active agents
                tmp = bincount(arr[:agentVision+1,:agentVision+1].flatten())
                if len(tmp)>2 and tmp[2]>maxx:
                    q = 0
                    maxx = tmp[2]
                tmp = bincount(arr[:agentVision+1,agentVision+1:].flatten())
                if len(tmp)>2 and tmp[2]>maxx:
                ...
                #Pick random spot in the quadrant
                xx = self.position[0]
                yy = self.position[1]

                grid[self.position] = 0
                position = ((xx+randint(quad[q][0],quad[q][1]))%gridSize,(yy+randint(quad[q][2],quad[q][3]))%gridSize)
                while (grid[position] != 0) and (grid[position] != 3):
                    position = ((xx+randint(quad[q][0],quad[q][1]))%gridSize,(yy+randint(quad[q][2],quad[q][3]))%gridSize)
                self.position = position
                grid[self.position] = self.active
...
########
quad = []
quad.append([-agentVision,0,-agentVision,0])
quad.append([-agentVision,0,0,agentVision])
quad.append([0,agentVision,0,agentVision])
quad.append([0,agentVision,-agentVision,0])
...
for iters in range(maxIter):
    ...
    #Go over list and active agents one by one
    for i in x:
        if i < len(Agents):
            Agents[i].act(agentVision,quad)
        else:
...
if MAKE_AWESOME_MOVIE:
    ...
    os.system("mencoder 'mf://Figures/_tmp*.png' -mf type=png:fps=5 -ovc lavc -lavcopts vcodec=wmv2 -oac copy -o animation.mpg")

print 'Time per iteration: ',(time.time() - start)/(maxIter+1)

#Save some data
savetxt('tmp/ac'+str(agentVision)+'_moda.dat',ac)
savetxt('tmp/ina'+str(agentVision)+'_moda.dat',ina)
savetxt('tmp/ja'+str(agentVision)+'_moda.dat',ja)
