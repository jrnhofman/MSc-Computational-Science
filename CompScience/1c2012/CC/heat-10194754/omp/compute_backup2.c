...
void do_compute(const struct parameters* p, struct results *r)
{
    ...
    double *localdiff = (double*) malloc(sizeof(double)*p->nthreads);
    for(i=0; i<p->nthreads; i++)
    {
	localdiff[i] = p->threshold+1;
    }
...
#pragma omp parallel \
    private(niter,row,column,cond,diff,localmax,localmin,q)
    {
	...
	*/
	...
	for(niter=1; (niter<(p->maxiter+1)) && (maxdiff>-1); niter++)
	{
	    localdiff[q] = 0.;
...
#pragma omp for nowait schedule(static)
		...
		for(column=1; column<M+1; column++)
		{
		    ...
		    if(localdiff[q] < diff) { localdiff[q] = diff; } 
		}
	    }
...
#pragma omp single 
		...
		for(i=0; i<p->nthreads; i++)
		{
		    r->maxdiff = fmax(r->maxdiff,localdiff[i]);
		}
